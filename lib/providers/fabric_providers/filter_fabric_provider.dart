...
class FilterFabricProvider extends ChangeNotifier {
  ...
  Future<void> getSyncData() async {
    ...
    blendId = fabricBlendsList
        .where((element) =>
            element.familyIdfk ==
            fabricFamilyList.first.fabricFamilyId.toString())
        .toList()
        .first
        .blnId;
    dataSynced = true;
  ...
  Future<List<FabricSetting>> getFabricSettingsData(String familyId) async {
    var dbInstance = await AppDbInstance().getDbInstance();
    fabricSetting = await dbInstance.fabricSettingDao
        .findFamilyFabricSettings(int.parse(familyId));
